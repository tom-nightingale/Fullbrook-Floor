...
class PH_Acquaint_XML_Import extends PH_Property_Import_Process {
	...
	public function __construct( $target_files = '', $instance_id = '' ) 
	{
	...
	public function import( $import_id = '' )
	{
        ...
        do_action( "propertyhive_pre_import_properties_acquaint_xml", $this->properties );
        $this->properties = apply_filters( "propertyhive_acquaint_xml_properties_due_import", $this->properties );
		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( 
					...
					($display_address != '' || (string)$property->descriptionbrief != '')
				)
				{
					$my_post = array(
				    	'ID'          	 => $post_id,
				    	'post_title'     => htmlentities(mb_convert_encoding(wp_strip_all_tags( $display_address ), 'UTF-8', 'ASCII'), ENT_SUBSTITUTE, "UTF-8"),
				    	'post_excerpt'   => htmlentities(mb_convert_encoding((string)$property->descriptionbrief, 'UTF-8', 'ASCII'), ENT_SUBSTITUTE, "UTF-8"),
				    	'post_content' 	 => '',
				...
				$office_id = $primary_office_id; // Needs mapping properly
				...
				if ( $department == 'residential-sales' )
				{
					...
					if ( isset($property->priceprefix ) && (string)$property->priceprefix  != '' )
					{
						...
						{
			                wp_set_post_terms( $post_id, $mapping[(string)$property->priceprefix], 'price_qualifier' );
			            }
			            else
						{
							$this->add_log( 'Property received with a price qualifier (' . (string)$property->priceprefix  . ') that is not mapped', (string)$property->id );

							$options = $this->add_missing_mapping( $mapping, 'price_qualifier', (string)$property->priceprefix, $import_id );
						}
			        }
				...
				elseif ( $department == 'residential-lettings' )
				{
					...
					{
		                wp_set_post_terms( $post_id, $mapping[(string)$property->rentaldetails->rentalfurnished], 'furnished' );
		            }
		        }
    				...
    				for ( $i = 0; $i <= 49; ++$i )
					{
						...
						if (isset($property->pictures) && !empty($property->pictures))
		                {
		                    for ($i = 1; $i <= 50; ++$i)
		                    {
		                        ...
		                        {
		                            $media_urls[] = array('url' => (string)$property->pictures->{"picture" . $i});
		                        }
		                    }
		                }
					}
    			...
    			else
    			{
	                ...
	                if (isset($property->pictures) && !empty($property->pictures))
	                {
	                    for ($i = 1; $i <= 50; ++$i)
	                    {
	                        if ( isset( $property->pictures->{"picture" . $i} ) && (string)$property->pictures->{"picture" . $i} != '' )
	                        {
	                            $media_urls[] = (string)$property->pictures->{"picture" . $i};
	                        }
	                    }
	                }
										...
										if ( 
											...
											(
												...
												(
													get_post_meta( $previous_media_id, '_modified', TRUE ) != ''
													&&
													get_post_meta( $previous_media_id, '_modified', TRUE ) == (string)$property->updateddate
												)
											)
										)
										{
								...
								else
								{
								    ...
								    else
								    {
									    ...
									    else
									    {
									    	...
									    	update_post_meta( $id, '_modified', (string)$property->updateddate);
				...
				if ( get_option('propertyhive_floorplans_stored_as', '') == 'urls' )
    			{
    				if ( isset($property->floorplan) && (string)$property->floorplan != '' )
	                {
						...
						{
							// This is a URL
		                    $media_urls[] = array('url' => (string)$property->floorplan);
		                }
					}
				...
				if ( get_option('propertyhive_brochures_stored_as', '') == 'urls' )
    			{
    				if ( isset($property->brochure) && (string)$property->brochure != '' )
	                {
						...
						{
							// This is a URL
		                    $media_urls[] = array('url' => (string)$property->brochure);
		                }
					}
				...
				if ( get_option('propertyhive_epcs_stored_as', '') == 'urls' )
    			{
	                ...
	                {
	                    $media_urls[] = array('url' => (string)$property->energyperformance->eerchart);
	                }
	                if (isset($property->energyperformance->eirchart) && (string)$property->energyperformance->eirchart != '')
	                {
	                    $media_urls[] = array('url' => (string)$property->energyperformance->eirchart);
	                }
				...
				do_action( "propertyhive_property_imported_acquaint_xml", $post_id, $property );
	...
	public function remove_old_properties( $import_id = '', $do_remove = true )
	{
		...
		if ( $property_query->have_posts() )
		{
			while ( $property_query->have_posts() )
			{
				...
				do_action( "propertyhive_property_removed_acquaint_xml", $post->ID );
			}
		}
		wp_reset_postdata();
