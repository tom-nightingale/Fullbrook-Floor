...
class PH_ReSales_Online_XML_Import extends PH_Property_Import_Process {
	...
	public function import( $import_id = '' )
	{
        ...
        do_action( "propertyhive_pre_import_properties_resales_online_xml", $this->properties );
        $this->properties = apply_filters( "propertyhive_resales_online_xml_properties_due_import", $this->properties );
		...
		foreach ( $this->properties as $property )
		{
			...
			if ( $inserted_updated !== false )
			{
				...
				if ( 
					...
					($display_address != '' || (string)$property->description->uk != '')
				)
				{
					$my_post = array(
				    	'ID'          	 => $post_id,
				    	'post_title'     => htmlentities(mb_convert_encoding(wp_strip_all_tags( $display_address ), 'UTF-8', 'ASCII'), ENT_SUBSTITUTE, "UTF-8"),
				    	'post_excerpt'   => htmlentities(mb_convert_encoding((string)$property->description->uk, 'UTF-8', 'ASCII'), ENT_SUBSTITUTE, "UTF-8"),
				    	'post_content' 	 => '',
				...
				if ( isset($property->type) )
				{
					...
					{
						wp_set_post_terms( $post_id, $mapping[(string)$property->type->uk . ' - ' . (string)$property->subtype->uk], $prefix . 'property_type' );
					}
					else
					{
						$this->add_log( 'Property received with a type (' . (string)$property->type->uk . ' - ' . (string)$property->subtype->uk . ') that is not mapped', (string)$property->id );

						$options = $this->add_missing_mapping( $mapping, $prefix . 'property_type', (string)$property->type->uk . ' - ' . (string)$property->subtype->uk, $import_id );
					}
				}
				...
				do_action( "propertyhive_property_imported_resales_online_xml", $post_id, $property );
	...
	public function remove_old_properties( $import_id = '', $do_remove = true )
	{
		...
		if ( $property_query->have_posts() )
		{
			while ( $property_query->have_posts() )
			{
				...
				do_action( "propertyhive_property_removed_resales_online_xml", $post->ID );
			}
		}
		wp_reset_postdata();
